...
int main(int argc,char **argv)
{
    ...
    /*初始化共享缓冲区*/
    camera_udp_shared_buffer.camera_data[0] = malloc(FRAME_HIGH * FRAME_WIDTH); // 1MB缓冲区
    camera_udp_shared_buffer.camera_data[1] = malloc(FRAME_HIGH * FRAME_WIDTH);
    camera_udp_shared_buffer.latest_index = 0;
    ...
    pthread_create(&t_camera_capture, NULL, camera_capture_thread, argv[1]);
    pthread_create(&t_udp_send, NULL, udp_send_thread, argv[2]);
